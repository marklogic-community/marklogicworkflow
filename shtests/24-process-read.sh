...
echo "starting 24-process-read.sh"
...
PID=`cat 23-process-create-out.txt`
#echo " THEPID: $THEPID PID: $PID"

curl -v --anyauth --user $MLADMINUSER:$MLADMINPASS -X GET \
    -H "Accept: application/xml" \
    "http://$RESTHOST:$RESTPORT/v1/resources/process?rs:processid=$PID" > 24-process-read-out.txt

echo "24-process-read.sh complete"
