...
import module namespace cc="http://marklogic.com/workflow/case-crud" at "/app/models/case-crud.xqy";
...
function ext:put(
  ...
  let $res := cc:case-create($input/ext:createRequest/ext:caseTemplateName/text(),
    $input/ext:createRequest/ext:data/element(),$input/ext:createRequest/ext:attachments/c:attachment,
    ()) (: Blank parent case id for now :)
...
) as document-node()*
{
  ...
  let $out :=
    if (fn:empty(map:get($params,"caseid"))) then
      ...
      <ext:readResponse><ext:outcome>SUCCESS</ext:outcome>
        {cc:case-get(map:get($params,"caseid"),fn:false())}
      </ext:readResponse>
